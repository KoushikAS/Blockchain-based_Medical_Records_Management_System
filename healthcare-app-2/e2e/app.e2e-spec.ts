...
describe('Starting tests for healthcare-app', function() {
  ...
  it('navbar-brand should be healthcare-app',() => {
    ...
    it('MedicalInfo component should be loadable',() => {
      page.navigateTo('/MedicalInfo');
      browser.findElement(by.id('assetName'))
      .then((assetName) => {
        return assetName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('MedicalInfo');
      });
    });

    it('MedicalInfo table should have 5 columns',() => {
      page.navigateTo('/MedicalInfo');
    ...
    it('UpdateVisit component should be loadable',() => {
      page.navigateTo('/UpdateVisit');
      browser.findElement(by.id('transactionName'))
      .then((transactionName) => {
        return transactionName.getText();
      })
      .then((txt) => {
        expect(txt).toBe('UpdateVisit');
      });
    });
